...
class Pawn(Piece):
    ...
    def availableMoves(self, x, y, gameboard, Color=None):
        ...
        # Normal single square forward move
        if (x, y + self.direction) not in gameboard:
            answers.append((x, y + self.direction))
            
            # First move two-square forward (if on starting row)
            if (self.Color == WHITE and y == 1) or (self.Color == BLACK and y == 6):
                if (x, y + 2 * self.direction) not in gameboard:
                    answers.append((x, y + 2 * self.direction))
        
        # Diagonal capturing move
        if (x + 1, y + self.direction) in gameboard and gameboard[(x + 1, y + self.direction)].Color != self.Color:
...
# Testcases
# Enter : a7 a6 to move a pawn
# Enter : a2 a3 to move a opponent's pawn
# Enter : b8 c6 to move a Knight